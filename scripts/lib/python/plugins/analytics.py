...
<div id="chart-Analytics">Analytics Chart</div>
<div id="chart-top_cpu">TOP_CPU Chart</div>
<div id="chart-top_mem">TOP_MEM Chart</div>
<iframe name="hiddenFrame" class="hide"></iframe>
<form method="post" action="/api/v1/set" enctype="application/json" target="hiddenFrame" onsubmit="gourl()">
<hr>
<legend>Configurations</legend>
<hr>
<br>Supress Zero values in top 5 charts:&nbsp;
    ...
    with open('/var/log/sys_status.log', 'rb') as csvfile:
        ...
        try:
            ...
            for row in csvdata:
                ...
                if bt and not bootFlag:
                    bootFlag=True
                    all_data['time'].append({"vLine": "true", "label": "Boot"})
                elif ts:
                    all_data['time'].append({"label": ts})
                all_data['cpu'].append({"value" : row[23-offset]})
                all_data['mem'].append({"value" : pct_get(row[26-offset], row[27-offset])})
                all_data['swap'].append({"value": pct_get(row[28-offset], row[29-offset])})
                all_data['top_cpu'].append({eval(row[1])[1] : float(eval(row[1])[2]),
                                            eval(row[3])[1] : float(eval(row[3])[2]),
                                            eval(row[4])[1] : float(eval(row[4])[2]),
                                            eval(row[5])[1] : float(eval(row[5])[2]),
                                            eval(row[6])[1] : float(eval(row[6])[2]),})
                all_data['top_mem'].append({eval(row[11])[1] : float(eval(row[11])[2]),
                                            eval(row[13])[1] : float(eval(row[13])[2]),
                                            eval(row[14])[1] : float(eval(row[14])[2]),
                                            eval(row[15])[1] : float(eval(row[15])[2]),
                                            eval(row[16])[1] : float(eval(row[16])[2]),})
                if ts is None:
                    break
        except Exception, e:
            print 'Failed2: %s' % e
...
class SystemStatusPlugin():
    def __init__ (self, exeCmd, vsize=135, hsize=140, temppath='./templates',
                  configFile='/etc/sys_system.conf'):
        with open(os.path.join(temppath,'ma_chart_script.xhtml')) as file:
            self.max_template = Template(file.read())
        with open(os.path.join(temppath,'ms_chart_script.xhtml')) as file:
            self.ms_template = Template(file.read())
        self.name           ='Analytics'
        self.plugType       = 'dial'
    ...
    def getHtmlDetail(self, pid=None):
        ...
        for ser in cpu_dataset:
            ...
            for ser_data in local_data['top_cpu']:
                if ser_data.get(ser['seriesname']) is not None and (self.zero_enabled == False or ser_data.get(ser['seriesname']) > 0):
                    top_cpu_data.append({"value": ser_data.get(ser['seriesname'])})
                else:
                    top_cpu_data.append({"value": ''})
        ...
        for ser in mem_dataset:
            ...
            for ser_data in local_data['top_mem']:
                if ser_data.get(ser['seriesname']) is not None and (self.zero_enabled == False or ser_data.get(ser['seriesname']) > 0):
                    top_mem_data.append({"value": ser_data.get(ser['seriesname'])})
                else:
                    top_mem_data.append({"value": ''})
        ...
        return (simpleHtml % (
            self.max_template.substitute(dict(name=self.name, 
                                       ...
                                       xAxis="Time",
                                       yAxis1NumPrefix=self.numPrefix,
                                       timestamps=ts,
                                       yAxis1Title="CPU Load",
                                       yAxis1NumberSuffix = '%',
                                       yAxis1Data=yAx1,
                                       yAxis2Title="Memory Free",
                                       yAxis2NumberSuffix = '%',
                                       yAxis2Max = 100,
                                       yAxis2Data=yAx2,
                                       yAxis3Title="SWAP Free",
                                       yAxis3NumberSuffix = '%',
                                       yAxis3Max = 100,
                                       yAxis3Data=yAx3,
                                       )),
            self.ms_template.substitute(dict(name='top_cpu', 
                                       vsize=500,
                                       hsize=900,
            ...
            self.ms_template.substitute(dict(name='top_mem', 
                                       vsize=500,
                                       hsize=900,
            ...
            'checked="checked"' if self.zero_enabled else '',
            'checked="checked"' if not self.zero_enabled else '',
            self.analyticsLimit
                            )
                )
